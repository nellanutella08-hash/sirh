...
import { renderRapportMensuelPdf } from "@/lib/rapportPdf";
...
export async function GET(req: NextRequest) {
  ...
  const pdf = renderRapportMensuelPdf(rapport);

  return new NextResponse(new Uint8Array(pdf), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="rapport-rh-${yearMonth}.pdf"`,
    },
  });
}
